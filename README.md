...
1. Extrai os dados públicos de CNPJs disponibilizados no portal do Ministério da Fazenda (https://arquivos.receitafederal.gov.br/dados/cnpj/dados_abertos_cnpj/);
2. Trata os dados construindo um dataframe sintetizado das informações;
3. Remove registros que não são de clínicas odontológicas;
4. Salva os dados tratados e limpos em parquet.

A figura abaixo ilustra o processo de execução no Airflow:

![fluxograma.png](docs%2Ffluxograma.png "Fluxograma de execução da Pipeline")
...
1. Garantir que o Airflow possui as dependências necessárias 
   2. É possível utilizar o Dockerfile para construir uma imagem customizada com as dependências já incluídas
2. Garantir que os recursos disponíveis para o Airflow são o suficiente para o processamento
   3. Mínimo 8GB Ram
